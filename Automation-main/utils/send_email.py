...
def _build_html_body(runs, agg, summary_path, dashboard_path):
    ...
    # Body advertises the attachment by its email-side filename (.txt), not
    # the on-disk filename (.md). Keeps the email body consistent with what
    # the recipient actually sees attached.
    if summary_basename.lower().endswith(".md"):
        summary_basename = summary_basename[:-3] + ".txt"
...
def send_run_email(runs, summary_path=None, dashboard_path=None,
    ...
    for path in (dashboard_path, summary_path):
        ...
        if attachment_name.lower().endswith(".md"):
            attachment_name = attachment_name[:-3] + ".txt"
        ctype, _ = mimetypes.guess_type(attachment_name)
        if ctype is None:
